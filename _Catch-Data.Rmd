...
3. Include information in the catch data that could be reasonably assessed by sampling crews in the field and is relevant to meeting fundamental or sub-objectives, and
4. Allow gear catchability to vary over a wide range of values in order to evaluate its effect on monitoring design outcomes.
...
catch data using two major steps:
...
consistent with past sampling proceedures.  In particular, we chose the number of sampled 
...
gear by basin (Table 2).  In general, gamma distributions provided a good fit to the effort data (e.g. Figure 1), but see Appendix ?? for more details.

[^2]: DEFINE STANDARD GEAR TYPES HERE


#### Catchability Distributions
Deployment catchability values ($q$), or the probability of catching a 
single fish with one unit of effort are a source of uncertainty.
...
as well as the number of gear deployments within a 
...
2007). 
  ...
  When a fish is successfully caught, fish id, location (bend), timing 
