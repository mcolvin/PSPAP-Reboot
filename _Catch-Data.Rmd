...
The catchability and capture probability of particular gears are 
important elements of simulating catch data. Since these values are 
highly uncertain and in some case unknown, we simulate catch data over a 
range of plausible gear catchabilities.
...
during which years) for various monitoring designs, 
2. Simulate 10 years of catch data for various sampling strategies using 
bends as the spatial scale, 
...
5. Simulate several catch data tables per combination of reference 
population and monitoring design. 
...
capture probability is abnormally high. 
...
caught by any other deployment within the same sampling occasion. For 
some gear types these assumptions are very likely met, while with others 
there are some caveats that we examine in THE DISCUSSION/APPENDIX ??. 
...
simulated catch data (Figure 2) using two major steps: 

1. For each year, select which bends to sample.
2. For each occasion, select which individual fish (of those located in 
the sampled bends) are caught. 
...
(CITATION) and reproduced in Table 1 below. The way selected bends were 
generated, however, varied and was determined by the choice of 
system-level sampling design laid out in the monitoring design. In 
...
hours) within a year during which the given bend was sampled. The fixed 
...
was set, dragged, or pulled for during a single deployment[^1]. Since 
...
standard gear types[^2] by basin. The mean effort, standard deviation of 
...
precisely known, we are currently accounting for size selectivity by 
comparing the catch data results simulated under various selction 
curves. See Appendix ?? for details. 
...
metrics for sub-objective 2[^3] as outlined in Section 4.1.1 of the 2016 
...
Table 2. Summary of effort data by gear and basin, where effort is 
