...
sample_bends<-matrix(0,10,nprim)
for(i in 1:nprim)
    {
    sample_bends[,i]<-sample(c(1:nbends),10,replace=FALSE)
    indx<- which(loc  %in% sample_bends[,i])    
    for(j in 1:nsec)
        {
        catch[indx,j,i]<-rbinom(length(indx),1,p*Z[,i]) 
        }
    }
...
NN<-colSums(Z)   
lens<-colSums(matrix(bend_km[sample_bends],10,nprim))
