...
out<-lapply(1:400, function(i)
{
  ...
  outi<-lapply(1:length(catch_list), function(j)
  {
    ...
    # SAVE ESTIMATES
    saveRDS(est,
            file=paste0("E:/_output/3-estimates/M0t_seg_est",
                        strsplit(catch_list[j], "catch_dat")[[1]][2]))
    return(est)
  })
...
#out1<-out[c(1:200)]
out2<-out[c(201:400)]
ests2<-do.call(rbind,lapply(out2, `[[`, 1))
COMBI2<-do.call(rbind,lapply(out2, `[[`, 2))
out_dat2<-list(ests=ests2, COMBI=COMBI2)
saveRDS(out_dat2, "E:/_output/3-estimates/M0t_seg_est_table_201-400.rds")
