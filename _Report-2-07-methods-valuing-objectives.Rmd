...
value, the better the design.  For monitoring design values to be as
meaningful and objective as possible, it is necessary for the valuation 
process to formally link the outcomes of alternative monitoring designs
to agency objectives with quantifiable metrics. The value of a 
monitoring design can then be thought of as an overall measurement of 
its utility, or usefulness, in achieving all objectives.  Quantifying 
such an overall monitoring design utility requires two steps:
...
metric values into a single objective utility.  For example, recruitment 
the detection power and estimator reliability metrics will be combined 
into an overall utility value for "Detect Recruitment".  
Before describing the process used to combine objective metrics, when needed, into a comprehensive objective utility, we first describe in detail the metrics used to quantify each of the 5 stakeholder objectives.
...
1. Estimate segment-level abundance, origin and stage specific
    1. bias
    1. precision
...
#### Combining fundamental objective metrics
The 3 metrics described above under trend can be combined into a single 
...
the plot of trend bias utility, $U_{T,bias}$, shown in Figure ??, trend esimates with smaller absolute bias are given higher utility values while increasingly large absolute bias values approach a utiliity of 0. 
...
0.30 is not very precise). The equation to calculate scaled precision is the 
same as for bias; however, we do not need to take absolute values since 
all CV values are positive. In Figure ??, trend  estimates with lower CV values 
have higher precision utility values, $U_{T,prec}$, and precision utility 
approaches 0 as CV values increase to the maximum of 0.30. 
...
discussion above may make valuating a monitoring program seem like 
daunting task, the structured decision making concepts and tools used in 
the process above provide an inclusive and transparent way of valuing 
monitoring designs that is linked to stakeholder objectives and values 
through quantifiable metrics and swing weighting.  Additionally, the use 
of Bayesian Decision Networks allows for the monitoring design 
