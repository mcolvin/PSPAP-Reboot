...
##### Minimum Known Alive
The minimum known alive (MKA) estimator
...
probability.  The $M_0$ estimator assumes that the capture probability 
is constant across all capture occasions ($p$).  The  $M_t$ estimator 
...
however, if fluctuation in capture probability are expected to be small, 
capture probabiliy may best be modelled as a constant with some 
environmental noise.  In this case, the $M_0$ estimator is likely the 
best choice of the two.   On the other hand, if different gears are used 
each occasion and we expect that the different gears will have a large 
difference in capture probability, then we can always use $M_t$ 
estimator.  Since in any of these cases, there may be a tradeoff between 
estimating more parameters (multiple capture probabilities) in order to 
reflect reality better, and the bias and precision of these estimates.  
Therefore, we implemented both the $M_0$ and the $M_t$ estimators in all 
scenarios and compare their outcomes.  

Both the $M_0$ and $M_t$ estimators rely on the assumption that the 
...
accross capture occasions (See ??).

The $M_0$ and $M_t$ estimators only analyze the data from within a year  
...
occasions nested with the primary sampling occasion (Figure 1). 
...
weighted mean approach described above.
